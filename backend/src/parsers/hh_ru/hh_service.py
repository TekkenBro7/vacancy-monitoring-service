...
class HHVacancyService(BaseVacancyService):
    ...
    async def _parse_range(self, query: str | None, start: datetime, end: datetime) -> int:
        ...
        if total >= hh_config.HH_MAX_TOTAL:
            ...
            if mid <= start:
                logger.warning("HeadHunter Cannot split further %s-%s", start, end)
                return 0

            logger.info(
                "HeadHunter Range %s - %s found %s vacancies, splitting",
                start,
                end,
                total,
            )
    ...
    async def run(
        ...
        while current < to_date:
            ...
            logger.info("HeadHunter %s → imported %s", current.date(), imported)
